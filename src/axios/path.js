import {fetchPost,fetchGet,fetchUpload,downloads} from '@/axios/http';
...
function downloadFile(data){
    return downloads("/dowload/downloadFile",data);
}
/* 打洞服务器 */
...
const allconfigs = {
    /**获取所有配置 */
    getAllConfigs() {
        return fetchGet("/configs/getAllConfigs")
    },
};
/* 用户接口 */
const user={
    /* 用户登录 */
    userLogin(data){
        return fetchPost("/login/pcloginWithTioNo",data);
    },
    /* 忘记密码 */
    retrievePwd(data){
    ...
    /**用户退出和登录时修改用户在线状态 */
    changeUserOnlineState(data) {
        return fetchPost("/chat/changeUserOnlineState",data);
    },
    /**获取用户信息 */
    getUserInfo(data) {
        return fetchGet("/user/info",data);
    },
...
const chatcom={
    ...
    /* 通讯录 */
    chatMailList(data){
        return fetchGet("/chat/pcmailList",data);
    },
    /* 聊天激活 */
    chatActChat(data){
